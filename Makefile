...
# Evaluation commands
eval:
	@echo "Running evaluation with Confident AI report"
	@bash -c "source scripts/set_env.sh ${ENV:-development} && deepeval login --confident-api-key ${CONFIDENT_AI_API_KEY} && \
		deepeval test run tests -m smoke"

eval-no-report:
	@echo "Running evaluation without generating report"
	@bash -c "source scripts/set_env.sh ${ENV:-development} && pytest tests -m smoke -vvvv"
...
help:
	...
	@echo "  eval: Run evaluation with Confident AI report"
	@echo "  eval-no-report: Run evaluation without generating a report"
	@echo "  clean: Clean up"
